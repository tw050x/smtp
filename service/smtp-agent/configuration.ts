...
const smtpServiceAgentLogLevelFilePath = resolve(__dirname, '.configuration/smtp-service.agent.log.level');
const smtpServiceAgentLogPathFilePath = resolve(__dirname, '.configuration/smtp-service.agent.log.path');
const smtpServiceAgentServerFileSizeFilePath = resolve(__dirname, '.configuration/smtp-service.agent.server.file-size');
...
if (existsSync(smtpServiceAgentLogLevelFilePath) === false) throw new Error('smtp-service.agent.log.level file not found');
if (existsSync(smtpServiceAgentLogPathFilePath) === false) throw new Error('smtp-service.agent.log.path file not found');
if (existsSync(smtpServiceAgentServerFileSizeFilePath) === false) throw new Error('smtp-service.agent.server.file-size file not found');
...
export const smtpServiceAgentLogLevel = readFileSync(smtpServiceAgentLogLevelFilePath, "utf-8").trim();
export const smtpServiceAgentLogPath = readFileSync(smtpServiceAgentLogPathFilePath, "utf-8").trim();
export const smtpServiceAgentServerFileSize = readFileSync(smtpServiceAgentServerFileSizeFilePath, "utf-8").trim();
...
export default {
  smtpDatabaseName,
  smtpDatabaseUri,
  smtpServiceAgentLogLevel,
  smtpServiceAgentLogPath,
  smtpServiceAgentServerFileSize,
  smtpServiceAgentServerHost,
  smtpServiceAgentServerPort,
  smtpServiceAgentServerTimeout,
}
